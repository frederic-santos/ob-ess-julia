...
** IN-PROGRESS A simple example of ~:session~ evaluation
...
- The ~:results value~ type of display is not supported yet:

  #+begin_src julia :results value :session *othersession* :exports both
root = Dict("marquise" = "orange", "eyrie" => "blue")
  #+end_src

  #+RESULTS:
  : You want to use :results value with :session *othersession*, but this does not work yet :-)

- Also, some displays are buggy and don't work yet, but this could be related to [[https://github.com/emacs-ess/ESS/issues/1053][an ESS bug]].
