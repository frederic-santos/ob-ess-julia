...
If a ~:session~ argument is given by the user in the header of a code block, this code block will be evaluated within a given Julia session, using ESS. Some simple examples are given below. (Note that Julia sessions must be started with a ~C-c C-c~ on a code block, not manually with ~M-x julia~.)
