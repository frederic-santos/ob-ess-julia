...
#+PROPERTY: header-args :tangle ob-julia.el
...
Il s'agit ici de donner le nom de la commande permettant de lancer une
session Julia.
...
** DONE Créer une nouvelle session si besoin [1/1]
   CLOSED: [2020-10-12 lun. 07:48]
...
** DONE Redirection générique : ~org-babel-julia-evaluate~
...
- Note taken on [2020-09-29 mar. 07:31] \\
  Il faut revenir sur =org-babel-variable-assignments:julia params)=
:END:
...
** IN-PROGRESS Évaluer un bloc de façon externe (session none) [4/5]
...
** IN-PROGRESS Évaluer à l'intérieur d'une session [4/7]
   ...
   - [X] Gérer les tables
   - [ ] Bug qui fait que certains blocs avec ~value~ doivent être
...
** DONE La fonction principale : ~org-babel-execute:julia~ [8/8]
...
- [X] Passer le ~body~ à la moulinette de ~org-babel-expand-body:julia~.
...
- [X] Rassembler les arguments pour produire les résultats et
  rediriger vers la fonction adéquate, ~org-babel-julia-evaluate~.
